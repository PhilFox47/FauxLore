...
export function FilterSortBar({ statusFilters, setStatusFilters, sortBy, setSortBy }: FilterSortBarProps) {
  ...
  return (
    <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3" style={{ '--accent': 'var(--accent, #f97316)' } as React.CSSProperties}>
      <div className="relative group flex-1" ref={dropdownRef}>
        <button
          onClick={() => setIsOpen(!isOpen)}
          className="w-full bg-zinc-900 border border-white/5 rounded-xl pl-9 pr-4 py-2 text-sm text-zinc-300 focus:outline-none focus:border-[var(--accent)] hover:border-white/10 transition-colors cursor-pointer text-left flex items-center justify-between"
        ...
        {isOpen && (
          <div className="absolute top-full left-0 mt-2 w-full bg-zinc-900 border border-white/10 rounded-xl shadow-xl z-50 overflow-hidden">
            <div className="p-1">
              ...
              >
                <div className={`w-4 h-4 rounded border flex items-center justify-center ${statusFilters.length === STATUSES.length ? 'bg-[var(--accent)] border-[var(--accent)]' : 'border-zinc-700 bg-zinc-800'}`}>
              ...
              {STATUSES.map(status => (
                <button
                  ...
                  className="w-full text-left px-3 py-2 text-sm text-zinc-300 hover:bg-white/5 rounded-lg transition-colors flex items-center space-x-2"
                >
                  <div className={`w-4 h-4 rounded border flex items-center justify-center ${statusFilters.includes(status) ? 'bg-[var(--accent)] border-[var(--accent)]' : 'border-zinc-700 bg-zinc-800'}`}>
      ...
      <div className="relative group flex-1">
        ...
        <select
          ...
          className="w-full bg-zinc-900 border border-white/5 rounded-xl pl-9 pr-8 py-2 text-sm text-zinc-300 focus:outline-none focus:border-[var(--accent)] appearance-none hover:border-white/10 transition-colors cursor-pointer"
          style={{ transition: 'border-color 0.2s' }}
        >
          ...
          <option value="ratingDesc">Lowest Rated</option>
        </select>
      </div>
    </div>
  );
